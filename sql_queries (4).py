...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays(songplay_id serial PRIMARY KEY,start_time timestamp,user_id varchar,level varchar, song_id varchar, artist_id varchar, session_id varchar, location varchar, user_agent varchar) """)
...
artist_table_create = ("""CREATE TABLE IF NOT EXISTS artists (artist_id varchar PRIMARY KEY , name varchar, location varchar, latitude real, longitude real)""")
...
song_select = ("""select a.artist_id, s.song_id from songs s join artists a on a.artist_id=s.artist_id where s.title=(%s) AND a.name=(%s) AND  s.duration=(%s)
""")
